...
bool check_boundaries(const Grid& g, const HeatConfig& cfg, double tol) {
    ...
    for (int j = 1; j < G-1; ++j) {
        if (std::abs(g.at(0,   j) - cfg.bc_left)   > tol) return false;
        if (std::abs(g.at(G-1, j) - cfg.bc_right)  > tol) return false;
    }
    // Skip corners (i=0, i=G-1)
    for (int i = 1; i < G-1; ++i) {
        if (std::abs(g.at(i, 0  ) - cfg.bc_bottom) > tol) return false;
        if (std::abs(g.at(i, G-1) - cfg.bc_top)    > tol) return false;
    }
    return true;
}
...
void ensure_dir(const std::string& path) {
    ...
    struct stat st{};
    if (stat(p.c_str(), &st) == 0) return;  // already exists
