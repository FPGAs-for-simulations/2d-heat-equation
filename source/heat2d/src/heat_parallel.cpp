...
static void thread_func(ThreadWork* w, const HeatConfig& cfg, int num_steps) {
    ...
    for (int k = 0; k < num_steps; ++k) {
        ...
        if (w->tid > 0) {
            ...
            std::memcpy(&below->u_old[their_ghost * G],
                        &w->u_old[w->ifirst * G],
...
// ============================================================
void solve_parallel(const HeatConfig& cfg, Grid& in_out, SolverMetrics& metrics) {
    const int N = cfg.N;
    ...
    for (int p = 0; p < m; ++p) worker_ptrs[p] = &workers[p];
    ...
    for (int p = 0; p < m; ++p) {
        ...
        // Copy initial grid into local buffer (local index li = global - row_start + 1)
        ...
        if (gi_first < 1)   w.ifirst += (1 - gi_first);
        if (gi_last  > N)   w.ilast  -= (gi_last - N);
    ...
    threads.reserve(m);
    for (int p = 0; p < m; ++p)
        threads.emplace_back(thread_func, &workers[p], std::cref(cfg), cfg.num_steps);
