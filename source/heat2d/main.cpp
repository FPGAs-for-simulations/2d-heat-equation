...
int main(int argc, char* argv[]) {
    HeatConfig cfg;
    // Override defaults to something that takes real time and shows parallel speedup.
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (!std::strcmp(argv[i], "-N")       && i+1 < argc) cfg.N              = std::atoi(argv[++i]);
        else if (!std::strcmp(argv[i], "-steps")   && i+1 < argc) cfg.num_steps      = std::atoi(argv[++i]);
        else if (!std::strcmp(argv[i], "-c")       && i+1 < argc) cfg.c              = std::atof(argv[++i]);
        else if (!std::strcmp(argv[i], "-threads") && i+1 < argc) cfg.num_threads    = std::atoi(argv[++i]);
        else if (!std::strcmp(argv[i], "-snap")    && i+1 < argc) cfg.snapshot_interval = std::atoi(argv[++i]);
        else if (!std::strcmp(argv[i], "-outdir")  && i+1 < argc) cfg.output_dir     = argv[++i];
        else if (!std::strcmp(argv[i], "-random"))                 cfg.use_random_ic  = true;
        else if (!std::strcmp(argv[i], "-ic")      && i+1 < argc) cfg.ic_value       = std::atof(argv[++i]);
        else if (!std::strcmp(argv[i], "-bl")      && i+1 < argc) cfg.bc_left        = std::atof(argv[++i]);
        else if (!std::strcmp(argv[i], "-br")      && i+1 < argc) cfg.bc_right       = std::atof(argv[++i]);
        else if (!std::strcmp(argv[i], "-bb")      && i+1 < argc) cfg.bc_bottom      = std::atof(argv[++i]);
        else if (!std::strcmp(argv[i], "-bt")      && i+1 < argc) cfg.bc_top         = std::atof(argv[++i]);
        else { std::fprintf(stderr, "Unknown option: %s\n", argv[i]); usage(argv[0]); return 1; }
    }

    std::printf("heat2d runner\n");
    std::printf("  N=%d  steps=%d  c=%.3f  dt=%.6f  ds=%.6f  r=%.4f\n",
                cfg.N, cfg.num_steps, cfg.c, cfg.dt(), cfg.ds(), cfg.r());
