...
static void test_metrics_sanity() {
  ...
  report("Serial   flop count correct", ms.total_flops == expected,
         ("got=" + std::to_string(ms.total_flops) +
          " expected=" + std::to_string(expected))
             .c_str());
  report("Parallel flop count correct", mp.total_flops == expected,
...
static void test_paper_example() {
  ...
  std::printf("  Compute speedup: %.3fx\n",
              ms.total_compute_ns /
                  (mp.total_compute_ns > 0 ? mp.total_compute_ns : 1));
}
...
static void test_thread_count() {
  ...
  for (int t : {2, 3, 4, 7}) {
    ...
    report(label, md < 1e-8, ("md=" + std::to_string(md)).c_str());
  }
}
